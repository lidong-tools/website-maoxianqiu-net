...
```text
consulting/treatment -> transferred
pending_payment/transferred -> hospitalized
consulting/treatment/hospitalized -> reopened
```
...
| 收费支付 | 结算 | `POST /api/payments` | `capture_payment` RPC |
...
| 调拨审核 | 审核通过 | `POST /api/transfers/:id/approve` | `approve_inventory_transfer` RPC |
