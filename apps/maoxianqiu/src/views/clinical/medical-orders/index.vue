...
import { MEDICAL_ORDER_STATUS_COLORS, MEDICAL_ORDER_STATUS_LABELS, MEDICAL_ORDER_TYPE_LABELS } from '@/types/clinical'
...
function getDataList() {
  ...
  apiClinical.listMedicalOrders({
    ...
    status: (search.value.status as MedicalOrderStatus) || undefined,
 ...
 * 走 create_medical_order RPC:单事务创建医嘱+任务+幂等+审计
 */
...
const tableColumns = computed<TableColumn<MedicalOrderRow>[]>(() => [
  ...
  },
  {
    ...
    cell: (info: any) => {
      ...
      const label = MEDICAL_ORDER_STATUS_LABELS[v as keyof typeof MEDICAL_ORDER_STATUS_LABELS] ?? v
      return h('span', { class: `px-2 py-0.5 rounded text-xs bg-${MEDICAL_ORDER_STATUS_COLORS[v as MedicalOrderStatus] ?? 'default'}-100` }, label)
    },
  },
  {
    id: 'operation',
      ...
      >
        ...
        <template #cell-operation="{ row }">
          <div class="flex-center gap-1">
            <FaButton v-if="row.original.status === 'active'" variant="outline" size="sm" @click="openCancel(row.original)">
              取消
            </FaButton>
          </div>
        </template>
